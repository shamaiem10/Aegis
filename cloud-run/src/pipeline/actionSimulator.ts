...
export interface SimulatedActionRow {
  action_id: string;
  before_state: string;
...
export function simulateResponseActions(
  category: string,
  ...
  const actions: SimulatedActionRow[] = [];

  actions.push({
    action_id: "traffic_reroute",
    before_state: `Corridor congestion index high around ${region}; EMS ETA elongated.`,
    response_action:
      "Staged detour + contraflow on feeder ramps; pause non-essential entries.",
  ...
  actions.push({
    ...
    response_action:
      "Pre-position EMS + cooling assets on downwind / downhill approach vectors.",
    expected_after_state:
      "Earlier triage contact; fewer late-stage heat / trauma presentations.",
  ...
  if (/flood|infrastructure|utility|waterlog/.test(cat)) {
    actions.push({
      ...
      before_state:
        "Unclear if inundation vs pressurized main breach — risk of wrong public hazard.",
      response_action:
        "Open utility war-room ticket; request pressure telemetry + valve isolation plan.",
      expected_after_state:
        "Narrow hypothesis space; safer correction path if flood alert was false.",
  ...
  if (/power|outage|grid/.test(cat)) {
    actions.push({
      ...
      response_action:
        "Rolling mobile gens + switching plan; staged re-energize hospitals first.",
      expected_after_state:
        "Critical loads stabilized within SLA; broader grid in phased restore.",
  ...
  if (/heat/.test(cat)) {
    actions.push({
      ...
      response_action:
        "Shelter activation + ORS distribution + staged fan / generator drops.",
  ...
  actions.push({
    ...
    response_action:
      "Geo-fenced push + SMS stub: instruct avoid vs staged evac language by confidence.",
