...
export async function dispatchAlert(alertId: string) {
  try {
    ...
    if (audienceType === 'PUBLIC' || audienceType === 'EMERGENCY_SERVICES') {
      ...
      const recipients: string[] = alert.emailRecipients || [];
