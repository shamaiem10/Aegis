...
function categoryFromSignal(s) {
    ...
    if (/flood|hydro|rain/.test(kind))
        return 'floods';
    if (/quake|seismic/.test(kind))
        return 'earthquakes';
    if (/health|disease|measles|dengue/.test(kind))
        return 'disease';
    if (/traffic|accident|road/.test(kind))
        return 'accidents';
    return 'general';
}
...
function ruleBasedTriage(signal) {
    const sev = Number(signal.severity_hint) || 5;
...
function ruleBasedCrisisAnalysis(signal, crisisId) {
    const sev = Number(signal.severity_hint) || 5;
    const cat = categoryFromSignal(signal);
    const region = signal.region || 'Pakistan';
    return {
        ...
        gaps: [cat, 'transport', 'public safety', sev >= 7 ? 'healthcare' : 'logistics'].filter(Boolean),
        recommendedActions: [
...
function runRuleBasedFallback(agentName, payload) {
    if (agentName === 'SignalFusionAgent') {
        const input = payload.input;
        const signals = input?.signals || [];
        const fusedSignals = signals.map((s) => {
            let credibilityScore = 50;
            const src = String(s.sourceType || s.source || '').toLowerCase();
            if (src === 'official' || src === 'pmd' || src === 'ndma')
                credibilityScore = 99;
            else if (src === 'weather' || src === 'open-meteo')
                credibilityScore = 94;
            else if (src === 'traffic' || src === 'here')
                credibilityScore = 91;
            return {
                ...s,
                credibilityScore,
    ...
    if (agentName === 'CombinedOrchestrationAgent') {
        const classOut = runRuleBasedFallback('CrisisClassificationAgent', payload);
        ...
        return {
            ...
            predictions: sev.predictions ?? [],
            allocations: alloc.allocations ?? [],
            falseAlarmChecks: fa.falseAlarmChecks ?? [],
            compoundRisks: compound.compoundRisks ?? [],
            alertDrafts: alerts.alertDrafts ?? [],
            note: 'rule-based combined fallback',
        };
    }
    const signal = signalFromPayload(payload);
    const crisisId = payload.input?.crisisId ||
        payload.input?.crisis?.crisis_id ||
        `pk-${signal.id}`;
