...
exports.pmdNDMAAlertsToParsedSignals = pmdNDMAAlertsToParsedSignals;
...
function regionsToLl(regions) {
    const pts = regions.map((r) => REGION_CENTROIDS[r]).filter(Boolean);
    if (!pts.length)
...
function pmdNDMAAlertsToParsedSignals(pmd, ndma) {
    const byId = new Map();
    for (const a of pmd) {
    ...
    return [...byId.values()].sort((a, b) => new Date(b.recorded_at).getTime() - new Date(a.recorded_at).getTime());
}
